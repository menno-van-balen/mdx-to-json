...
export default function extract(mdx) {
  ...
  const mdxJson = JSON.stringify(originalMdxTree, null, 2);
  const translationJson = JSON.stringify(translationObject, null, 2);
  fs.writeFile(`${fileName}.mdx-tree.json`, mdxJson, (er) => console.log(er));
  fs.writeFile(`${fileName}.json`, translationJson, (er) => console.log(er));
}
