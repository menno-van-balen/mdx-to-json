...
function createTranslationObject(tmxTree, keyId) {
  const transObject = {};
    ...
    if (node.type === "yaml") {
      node.attributes.forEach((atr, j) => {
        if (
          ...
          atr.name === "title_post" ||
          atr.name === "title_html" ||
          atr.name === "hero_image_alt" ||
          atr.name === "description"
        ) {
          const key = `${i}.${j}-${keyId}-${node.type}-${atr.name}`;
          ...
          atr.value.forEach((val, k) => {
            val.forEach((el, l) => {
              if (
                ...
                const key = `${i}.${j}.${k}.${l}-${keyId}-${node.type}-${el.name}`;
      ...
      if (children.type === "text") {
        const key = `${i}-${keyId}-${node.type}`;
        transObject[key] = children.value;
      } else if (children.type === "image") {
        const key = `${i}-${keyId}-${children.type}`;
      ...
      node.children.forEach((child, j) => {
        const key = `${i}.${j}-${keyId}-${node.type}`;
        let value = "";
        if (child.children.length === 1) {
          value = child.children[0].value;
        } else {
          const grantChildren = child.children;
          for (let k = 0; k < grantChildren.length; k++) {
            if (grantChildren[k].type === "break") {
              break;
            } else if (grantChildren[k].type === "text") {
              value += grantChildren[k].value;
            } else {
              value += `<g-${k}>${grantChildren[k].value}</g>`;
            }
          }
        }
        transObject[key] = value;
...
function createOriginalMdxTree(mdx) {
  ...
  tree.children.forEach((el) => {
    ...
    if (el.type === "yaml") {
    ...
    } else if (el.type === "mdxjsEsm") {
      const node = {
        type: el.type,
        value: el.value,
      };
      tmxTree.nodes.push(node);
    } else if (el.name === "Carousel") {
      ...
      tmxTree.nodes.push(node);
    } else if (el.type === "mdxJsxFlowElement" && el.name !== "Carousel") {
      // console.log(el)
      ...
      el.children.forEach((child) => {
        const childNode = createMdTree(child);
      ...
      el.children.forEach((child) => {
        child.children.forEach((grantChild) => {
          const grantChildNode = createMdTree(grantChild);
          listNode.children.push(grantChildNode);
        });
      });
      tmxTree.nodes.push(listNode);
    } else if (el.type === "paragraph") {
      const node = createMdTree(el);
      tmxTree.nodes.push(node);
    } else if (el.type === "thematicBreak") {
      const node = {
        type: el.type,
      };
      tmxTree.nodes.push(node);
      ...
      // console.log(el);
      console.log("Message from createOriginalMdxTree: New element found!");
    }
  });
  return tmxTree;
}
